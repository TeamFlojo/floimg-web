...
The floimg plugin provides six slash commands. Simple commands use CLI and work immediately. Complex commands use MCP for session state and iteration.

## Simple Commands (CLI)

These commands work immediately after installing the plugin - no MCP setup required.
...
These commands require MCP for session state and iteration. Restart Claude Code once after installing the plugin to enable MCP.
